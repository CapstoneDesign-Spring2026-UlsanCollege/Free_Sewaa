# Representative PRs and Commits

## Feature Branches

The following branches represent key development work:

- `feature/backend-api` — Backend API implementation
- `feature/browse-donation-items` — Browse items feature
- `feature/browse-ui` — Browse UI components
- `feature/homepage-ui` — Homepage design
- `refactor/react-migration` — React frontend migration

## Copilot-Assisted Branches

- `copilot/add-donate-page` — Donate page with AI assistance
- `copilot/build-vertical-slice-post-browse-request` — Vertical slice implementation
- `copilot/improve-agent-performance` — Performance improvements
- `copilot/improve-project-quality` — Quality improvements
- `copilot/week-3-mvp-implementation` — Week 3 MVP with AI assistance

## Key Commits

- [7dc196f](https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/7dc196f) — Convert frontend pages to React
- [b38c79d](https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/b38c79d) — Ensure React build runs on Render start
- [9614646](https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/9614646) — Preload React page styles
- [7399ae8](https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/7399ae8) — Match homepage to Figma reference
- [d26a904](https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/d26a904) — Match browse page to Figma marketplace
- [807aff0](https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/807aff0) — Fix auth header scroll layout

## Complete Commit History

Full commit history: [main branch commits](https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commits/main)

[Back to AI and Code Ownership](../README.md)

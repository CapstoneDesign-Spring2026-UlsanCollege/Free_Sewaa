# Sprint Packet — Sprint 1

## MVP Week 1
[docs/PROGRESS/MVP_WEEK_1.md](https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/blob/9000e164525de9c5cdba954002e6409ec2a6986a/docs/PROGRESS/MVP_WEEK_1.md)

## Team

**Team Name:** Free Sewaa  
**Repository:** https://github.com/CapstoneDesign-Spring2026-UlsanCollege/freesewaa  
**Sprint:** Sprint 1 — Week 1 (Onboarding)

### Sprint Roles

| Role            | Team Member        |
|-----------------|-------------------|
| Project Manager | Sujan Shrestha    |
| Scribe          | Sujan Tamang      |
| QA Lead         | Swarnim Jung Karki|
| Demo Driver     | Ram Pathak        |
| Developer       | Mohan Khadka      |

The Project Manager coordinates sprint activities and prepares the Sprint Packet submission.

---

# Demo

**Demo Link:** <img width="1257" height="619" alt="image" src="https://github.com/user-attachments/assets/6803f2a3-8f07-4993-bc71-a8b70593c0a6" />
  
**Backup Demo:** 

https://github.com/user-attachments/assets/88e673f3-80bf-4409-a4fc-7696fd4c65a7

### Demo Script

- Introduce **Free Sewaa**, a community donation and resource-sharing platform that helps reduce waste by connecting donors with people who need reusable items.
- Show the **GitHub repository**, including the project structure and documentation files prepared during Sprint 1.
- Show the **project idea pitch issue** and **team agreement** as evidence of onboarding progress.
- Demonstrate the **initial homepage interface** built with HTML and CSS as the team’s first technical artifact.

---

# Backup Demo Plan

If the live demo does not work, the team will present:

- Screenshot of the GitHub repository
- Screenshot of the project idea pitch issue
- Screenshot of the team agreement
- Screenshot of the homepage interface
- Recorded walkthrough video if available

---

# Board Snapshot

### To Do
- Configure frontend development environment
- Set up backend project structure
- Prepare issue templates

### Doing
- Preparing project documentation
- Planning system architecture

### Done
- Team repository created
- Project idea pitch issue opened
- Team agreement started
- Sprint Packet Issue created
- Initial homepage interface created

---

# Owners

| Responsibility                    | Owner               |
|----------------------------------|---------------------|
| Sprint coordination              | Sujan Shrestha      |
| Documentation and notes          | Sujan Tamang        |
| Quality review and verification  | Swarnim Jung Karki  |
| Demo preparation and presentation| Ram Pathak          |
| Homepage interface development   | Mohan Khadka        |

---

# Definition of Done

Sprint 1 is considered complete when:

- The team repository has been created
- The team agreement has been started
- The project idea pitch issue has been opened
- The Sprint Packet Issue has been created
- A small working technical artifact has been added to the repository
- Each team member has posted at least 2 contribution receipts

---

# Sprint Notes

## What Shipped

During Sprint 1, the team successfully completed the onboarding phase of the Free Sewaa project.

Completed work includes:
- Creating the team repository
- Opening the project idea pitch issue
- Starting the team agreement
- Creating the Sprint Packet Issue
- Building an initial homepage interface using HTML and CSS

This sprint established the foundation for future feature development and team collaboration.

## What Broke

During repository setup, the team faced a synchronization conflict while pushing files to GitHub because the remote repository already contained existing files.

## How It Was Fixed

The issue was resolved by pulling the latest remote changes, syncing the local repository, and then pushing the updated files successfully.

## Next Sprint Plan

During Sprint 2 (Reset Week), the team will focus on:

- Organizing the GitHub project board
- Setting up issue templates
- Finalizing frontend and backend development environments
- Beginning development of the item listing interface

## Risks / Blockers

Possible challenges include:
- Some team members may need help configuring development environments
- Scheduling team meetings may be difficult
- Backend technology decisions may take additional discussion

The team will manage these risks through communication and collaborative troubleshooting.

---

# Individual Contribution Receipts

Each team member must comment on this Sprint Packet Issue with 2–3 pieces of evidence of their work.

Examples:
- Pull requests
- Git commits
- Issues created or completed
- Documentation updates
- Screenshots of progress



**Individual Contribution Receipts**


PATHAK RAM
**Commit:**  
- Commit: https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/f0e8084
- Commit: https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/0ce9d10

  **PR:**  
- Pull Request: https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/pull/13
  
**Issue Created / Updated:**
[https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/issues/9#issue-4092354028]



sujan shrestha 
**Commits:**  
- https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/0ce9d107259208262363638bbf65902626c9cae3 — Update WEEK_1.md
- https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/f0e80848f9af3296a5a03cdba3b30872f860d3c5 — Update WEEK_1.md

**PR:**  
- https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/pull/14 — Sprint 1 contributions
  
**Issue Created / Updated:**
- https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/issues/51#issue-4198526690

Sujan Tamang
**Commits:**   
- https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/d057e13bb2fc2ea22424b83983205d78d9d14381
- https://github.com/CapstoneDesign-Spring2026-UlsanCollege/Free_Sewaa/commit/a0ed22f2833f8777ea7e11f386ed5381a2053f42

**Documentation Update:**  
Updated PROJECT.md / TEAM_AGREEMENT.md / README.md

---


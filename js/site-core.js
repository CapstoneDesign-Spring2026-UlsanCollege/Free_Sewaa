(function () {
  ...
  function initChatbotWidget() {
    ...
    function localChatbotReply(text) {
      const lower = String(text || '').toLowerCase();
      if (/\b(message|chat|contact|talk)\b/.test(lower)) {
        return 'Open Messages from your dashboard or from an item request. Choose a conversation, type your message, and press Send.';
      }
      if (/\b(donate|post|upload|give)\b/.test(lower)) {
        return 'To donate, open Donate, add the item title, category, condition, pickup window, notes, and a clear photo. Then publish the listing.';
      }
      if (/\b(request|claim|interested|reserve)\b/.test(lower)) {
        return 'To request an item, open Browse, preview the item, then press Request. You can continue from Messages after the donor replies.';
      }
      if (/\b(pickup|location|meet|safe)\b/.test(lower)) {
        return 'For pickup, use a public location, agree on a clear time, and keep exact address details inside Messages.';
      }
      return 'I can help with donating items, browsing, requests, pickup safety, messages, profile settings, and saved items. What do you want to do?';
    }
    ...
    async function sendChatMessage(text) {
      ...
      try {
        ...
        const reply = data.reply || localChatbotReply(text);
        pending.querySelector('p').textContent = reply;
        history.push({ role: 'bot', text: reply });
        saveHistory(history);
      } catch (error) {
        const fallback = localChatbotReply(text);
